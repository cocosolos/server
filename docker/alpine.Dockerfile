...
ENV PATH="$VIRTUAL_ENV/bin:/xiadmin/.luarocks/bin:$PATH"
...
RUN luarocks --lua-version 5.1 --local install https://luarocks.org/manifests/lunarmodules/busted-scm-1.rockspec
USER root
...
MARIADB_FILE="/xiadmin/build/_deps/mariadb-connector-cpp-src/include/conncpp/CArray.hpp"
if [ -f "$MARIADB_FILE" ]; then
        if ! grep -qF '#include <cstdint>' $MARIADB_FILE; then
        echo "Patching $MARIADB_FILE: Adding #include <cstdint>"
        sed -i '1i #include <cstdint>' $MARIADB_FILE
    fi
else
    echo "Warning: $MARIADB_FILE not found, skipping patch."
fi
...
COPY --chown=$UNAME:$UGROUP --from=staging /xiadmin/.luarocks /xiadmin/.luarocks
