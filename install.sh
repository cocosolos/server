...
then
    ...
    sudo apt install git python3 python3-pip g++-10 cmake make libluajit-5.1-dev libzmq3-dev libssl-dev zlib1g-dev mariadb-server libmariadb-dev binutils-dev || exit
elif [ $OS = "arch" ] || [[ $OS_LIKE =~ $ARCH ]]
then
    sudo pacman -S git python3 python-pip gcc cmake make luajit zeromq openssl zlib binutils || exit
